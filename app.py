...
top_left, top_right = st.columns([15, 3])
    ...
    if st.session_state.authenticated and st.session_state.user:
        ...
        st.markdown(
            ...
            <div style="
                ...
                padding-top: 8px;
            ">
                👋 Welcome, <span style="color:#4c78a8">{username}</span>
            </div>
            """,
...
if st.session_state.show_auth and not st.session_state.authenticated:
    ...
    # st.markdown("Access advanced features by signing in")
    ...
    with auth_col2:
        st.subheader(
            "🔐 Login" if st.session_state.auth_mode == "Login"
            else "📝 Create Account"
        )
...
# PAGE 5: EARLY WARNING
    ...
    if not st.session_state.authenticated:
        st.warning("🔐 Please login to access alerts")
        st.stop()
